...
describe('<Accordion/>', () => {
  it('should render accordion with popup data', () => {
  ...
  it('should open & close text', () => {
    const { queryByText, getByText, container, debug, findByText } = render(
      <Accordion>
        <Accordion.Title>Frequently Asked Questions</Accordion.Title>
        <Accordion.Frame>
          {faqsData.map((item) => (
    ...
    expect(queryByText('hiddenText')).toBeFalsy();
    fireEvent.click(getByText('What is Netflix?'));
    expect(findByText('hiddenText')).toBeTruthy();
